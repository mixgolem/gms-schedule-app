    ...
    <div className="border rounded-lg p-3 transition-shadow duration-150 hover:shadow-sm">
      ...
      {canEdit && (
        <p className="text-[11px] text-blue-600 mb-2">
          <span className="inline-block rounded px-1 bg-blue-50/50 border border-dashed border-blue-200 mr-1">
            숫자 ✎
          </span>
        ...
        <table className="text-xs w-full">
          <thead>
            <tr className="text-left text-blue-900">
              ...
              {MONTH_ORDER.map((m) => (
                <th key={m} className="pb-1 px-2 font-normal text-right">
                  {m}월
                </th>
              ))}
              <th className="pb-1 px-2 font-normal text-right whitespace-nowrap">대휴누적</th>
              <th className="pb-1 px-2 font-normal text-right whitespace-nowrap">사용누적</th>
              <th className="pb-1 px-2 font-normal whitespace-nowrap">사용가능</th>
              <th className="pb-1 px-2 font-normal text-right whitespace-nowrap">당월사용(시간)</th>
              <th className="pb-1 px-2 font-normal whitespace-nowrap">당월사용(날짜)</th>
              <th
                className="pb-1 px-2 font-normal text-right whitespace-nowrap cursor-help"
          ...
          <tbody>
            {rows.map((row, i) => (
              ...
              >
                ...
                <td className="py-1 px-2 text-right">{row.accruedTotal}</td>
                ...
                <td className="py-1 px-2 text-right">{row.usedThisMonthHours || ""}</td>
                <td className="py-1 px-2 whitespace-nowrap">{row.usedThisMonthDates.join(", ")}</td>
                <td className="py-1 px-2 text-right text-gray-500">
                  {row.autoAccrualThisMonth || ""}
                </td>
              </tr>
            ))}
          </tbody>
